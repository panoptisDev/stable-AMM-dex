...
import { checkStableAMMToken } from '../../functions'
...
export class Pair {
  ...
  public getOutputAmount(
    ...
    let standardAMMType = true
    try{
      if (checkStableAMMToken(this.token0, this.token1)) standardAMMType = false
    }
    catch(err) {
      standardAMMType = true
    }
    if (standardAMMType){
      ...
      const inputAmountWithFee = JSBI.multiply(inputAmount.quotient, _997)
      const decimalDifference = inputAmount.currency.decimals - (inputAmount.currency.equals(this.token0) ? this.token1 : this.token0).decimals
      const outputAmount = CurrencyAmount.fromRawAmount(
        inputAmount.currency.equals(this.token0) ? this.token1 : this.token0,
        JSBI.divide(JSBI.divide(inputAmountWithFee, _1000), 
  ...
  public getInputAmount(
    ...
    if (standardAMMType){
      ...
      const decimalDifference = outputAmount.currency.decimals - (outputAmount.currency.equals(this.token0) ? this.token1 : this.token0).decimals
      const inputAmount = CurrencyAmount.fromRawAmount(
        outputAmount.currency.equals(this.token0) ? this.token1 : this.token0,
        JSBI.divide(JSBI.divide(outputAmount.quotient, _997), 
        JSBI.exponentiate(JSBI.BigInt(10), JSBI.BigInt(decimalDifference-3)))
      )
      return [
        inputAmount,
        new Pair(
