...
export function useUserInfo(farm, token) {
  ...
  const contract = useBeamVaultContract()
...
export function usePendingTokens(farm) {
  ...
  return amount ? CurrencyAmount.fromRawAmount(GLINT[chainId], amount) : undefined
}
...
export function useGlintPositions(contract?: Contract | null) {
  ...
  return useMemo(() => {
    if (!pendingGlint || !userInfo || !userLockedUntil) {
      return []
    }
    return zip(pendingGlint, userInfo, userLockedUntil)
      .map((data, i) => ({
        id: args[i][0],
      ...
      .filter(({ pendingGlint, amount }) => {
        return (pendingGlint && !pendingGlint.isZero()) || (amount && !amount.isZero())
      })
  }, [args, pendingGlint, userInfo, userLockedUntil])
}

export function usePositions() {
  return useGlintPositions(useBeamVaultContract())
}

export function useBeamVaults(contract?: Contract | null) {
  const numberOfPools = useSingleCallResult(contract ? contract : null, 'poolLength', undefined, NEVER_RELOAD)
    ?.result?.[0]
  ...
  return useMemo(() => {
    ...
    return zip(poolInfo).map((data, i) => ({
      ...
      accGlintPerShare: data[0].result?.['accSolarPerShare'] || '',
...
export function useVaults() {
  return useBeamVaults(useBeamVaultContract())
}
...
export function useglintPrice() {
  // eslint-disable-next-line react-hooks/rules-of-hooks
  return usePrice(useglintGlimmerContract())
}
...
export function useVaultInfo(contract) {
  const glintPerBlock = useSingleCallResult(contract ? contract : null, 'solarPerBlock', undefined, NEVER_RELOAD)
    ?.result?.[0]
...
export function useBeamVaultInfo() {
  return useVaultInfo(useBeamVaultContract())
}
