...
import { BEAMCHEF_ADDRESS, BEAM_VAULT_ADDRESS } from '../../constants/addresses'
...
const VaultListItem = ({ farm }) => {
  ...
  const [approvalState, approve] = useApproveCallback(typedDepositValue, BEAM_VAULT_ADDRESS[chainId])
    ...
    <>
      <Modal isOpen={showConfirmation} onDismiss={() => setShowConfirmation(false)}>
        <div className="space-y-4">
          ...
          <Typography variant="lg" className="font-medium pt-4">
            {i18n._(
              t`${
                currentAction.action == 'deposit' ? 'Staking on Glint Vault' : 'Harvesting'
      ...
      >
        <Disclosure.Panel className="flex flex-col w-full border-t-0 rounded rounded-t-none bg-dark-800" static>
          ...
          {pendingGlint && pendingGlint.greaterThan(ZERO) && (
            <div className="px-4 pb-4">
              ...
              >
                {i18n._(t`Harvest ${formatNumber(pendingGlint.toFixed(18))} GLINT`)}
              </Button>
            </div>
          )}
        </Disclosure.Panel>
