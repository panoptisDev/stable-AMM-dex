...
export function useChefContract(chef: Chef) {
  const beamDistributorContract = useShareFarmContract()
...
export function useGlintPositions(contract?: Contract | null) {
  ...
  const numberOfPools = useSingleCallResult(contract ? contract : null, 'stakeInfoLen', undefined, NEVER_RELOAD)
    ?.result?.[0]
  ...
  const pendingGlint = useSingleContractMultipleData(args ? contract : null, 'pendingReward', args)
  ...
  return useMemo(() => {
    ...
    return zip(pendingGlint, userInfo)
      .map((data, i) => ({
        id: args[i][0],
        pendingGlint: data[0].result?.[0] || Zero,
...
export function usePoolRewardsPerSec(contract?: Contract | null) {
  ...
  const args2 = useMemo(() => {
    ...
    return [...Array(numberOfPools.toNumber()).keys()].map((pid) => [String(pid), String(pid)])
  }, [numberOfPools])

  const pendingGlint = useSingleContractMultipleData(args ? contract : null, 'stakeinfoRewardInfo', args2)
  const userInfo = useSingleContractMultipleData(args ? contract : null, 'userInfo', args)
  ...
  return useMemo(() => {
    ...
    return zip(pendingGlint).map((data, i) => ({
      id: args[i][0],
      rewardsPerSec: data[0].result?.rewardPerSec || Zero,
    }))
  }, [args, pendingGlint])
}

export function usePositionsShare() {
  return useGlintPositions(useShareFarmContract())
}

export function useRewardsPerSecPosition() {
  return usePoolRewardsPerSec(useShareFarmContract())
}
...
export function useGlintFarms(contract?: Contract | null) {
  ...
  const poolInfo = useSingleContractMultipleData(args ? contract : null, 'stakeInfo', args)
  ...
  return useMemo(() => {
    ...
    return zip(poolInfo).map((data, i) => ({
      id: args[i][0],
      lpToken: data[0].result?.['stakingToken'] || '',
      rewardToken: data[0].result?.['rewardToken'] || '',
      precision: data[0].result?.['precision'] || '',
      startTimestamp: data[0].result?.['startTimestamp'] || '',
      lastRewardTimestamp: data[0].result?.['lastRewardTimestamp'] || '',
      accRewardPerShare: data[0].result?.['accRewardPerShare'] || '',
      totalStaked: data[0].result?.['totalStaked'] || '',
      totalRewards: data[0].result?.['totalRewards'] || '',
    }))
  }, [args, poolInfo])
}
...
export function useShareFarms() {
  return useGlintFarms(useShareFarmContract())
}
...
export function usePricesApi() {
  ...
  }, [glmrPrice, ribPrice, glintPrice, usdcPrice, ethPrice, wbtcPrice, bnbPrice, beefyPrice, fantomPrice])
}
...
export function useBeamDistributorInfo(contract) {
  ...
  const args = useMemo(() => {
    ...
    return [...Array(numberOfPools.toNumber()).keys()].map((pid) => [String(pid), String("0")])
  }, [numberOfPools])

  const glintPerBlock = useSingleContractMultipleData(
  ...
  return useMemo(() => {
    ...
    return zip(glintPerBlock).map((data, i) => ({
      id: args[i][0],
      rewardsPerSec: data[0].result?.rewardPerSec || Zero,
      endTimestamp: data[0].result?.endTimestamp || Zero,
    }))
  }, [args, glintPerBlock])
}

export function useDistributorInfoShare() {
  return useBeamDistributorInfo(useShareFarmContract())
}
