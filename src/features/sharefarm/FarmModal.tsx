...
const YieldDetails: React.FC<FarmModalProps> = ({ isOpen, onDismiss, farm }) => {
  ...
  const totalPoolTokens = farm?.totalStaked
  const userTokens = parseFloat(amount?.toSignificant(4))
  const totalTokens = parseFloat(ethers.utils.formatEther(farm?.totalStaked))
  const poolTokenPercentage = (userTokens * totalTokens) / 100
  ...
  const [approvalState, approve] = useApproveCallback(typedDepositValue, SHARE_ADDRESS[chainId])

  // allowance handling
  const { gatherPermitSignature, signatureData } = useV2LiquidityTokenPermit(typedDepositValue, SHARE_ADDRESS[chainId])
          ...
          <div className="first md:w-3/5 bg-blue ml-3 mr-3 md:mr-0" style={{ border: '1px solid #1F357D' }}>
            ...
            <div className="px-4 py-2 flex-col md:flex md:flex-row justify-between text-xs bg-deepCove">
              <div className="flex-col md:flex md:flex-row justify-between md:justify-start md:flex-col ml-3 mt-3 md:mt-0 pb-2 md:pb-0">
                ...
                <div className="flex justify-between md:flex-col ml-0">
                  ...
                  <div className="text-white font-bold text-lg pb-1 mt-3 mr-3 md:mt-0 md:mr-0">
                    {poolTokenPercentage > 0 ? poolTokenPercentage?.toFixed(3) + '%' : '0%'}
                  </div>
                </div>
              </div>
              ...
              <div className="flex-col md:w-2/3 bg-darkBlue p-6 m-3 rounded-sm">
                ...
                <div className="text-white font-bold text-lg pb-1">
                  {pendingGlint[0]?.length <= 1 && i18n._(t`${formatNumber(pendingGlint[0].toFixed(18))}`)}

                  {!account && '0'}
                </div>
                <div className="text-jordyBlue font-bold text-xs">
                  {pendingGlint[0]?.length <= 1 &&
                ...
                {pendingGlint >= 1 && (
                  <div className="mt-4 pb-4">
                    ...
                    >
                      {pendingGlint[0]?.length <= 1 && i18n._(t`Harvest`)}
                    </Button>
                  </div>
                )}
              </div>
