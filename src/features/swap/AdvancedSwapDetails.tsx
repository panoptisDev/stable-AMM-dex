...
export function AdvancedSwapDetails({ trade, allowedSlippage, minerBribe }: AdvancedSwapDetailsProps) {
  ...
  return !trade ? null : (
    <div className="flex flex-col space-y-2 bg-deepCove p-3 rounded-md">
      <div className="flex flex-row items-center justify-between">
        <span className="flex items-center">
          <div className="text-sm text-jordyBlue">{i18n._(t`Route`)}</div>
      ...
      <RowBetween>
        <RowFixed>
          <div className="text-sm text-jordyBlue">
      ...
      {/* <RowBetween>
       <RowFixed>
         <div className="text-sm text-jordyBlue">{i18n._(t`Price Impact`)}</div>
         <QuestionHelper
      ...
      <RowBetween>
        <RowFixed>
          <div className="text-sm text-jordyBlue">{i18n._(t`Trading Fee`)}</div>
        </RowFixed>
        <div className="text-sm font-bold text-high-emphesis">
          {realizedLPFee
            ? `${formatNumberScale(realizedLPFee.divide(6).multiply(1307692308).divide(1000000000).toSignificant(4))} ${
      ...
      {/* <RowBetween>
        <RowFixed>
          <div className="text-sm text-jordyBlue">{i18n._(t`xSUSHI Fee`)}</div>
          <QuestionHelper
      ...
      <RowBetween>
        <RowFixed>
          <div className="text-sm text-jordyBlue">{i18n._(t`Slippage tolerance`)}</div>
        </RowFixed>
      ...
      {minerBribe && (
        <RowBetween>
          <RowFixed>
            <div className="text-sm text-jordyBlue">{i18n._(t`Miner Tip`)}</div>
