...
import { checkStableAMMCurrency } from '../../functions'
...
export default function TradePrice({ price, showInverted, setShowInverted, className, trade }: TradePriceProps) {
  ...
  const standardAMMType = checkStableAMMCurrency(price.quoteCurrency, price.baseCurrency)
  const ammTypeText = standardAMMType ? "Stable AMM" : "Standard AMM"
    ...
    <div>
      <div
        ...
        className={classNames(
          'flex justify-between cursor-pointer w-full px-5 py-2 cursor-pointer text-jordyBlue',
          className
        )}
      >
      ...
      <div className={classNames('flex justify-between w-full px-5 py-2 rounded-b-md text-jordyBlue', className)}>
        <Typography variant="sm" className="select-none">
        ...
        <Typography variant="sm" className="select-none text-aqua">
      ...
      <div className={classNames('flex justify-between w-full px-5 py-2 rounded-b-md text-jordyBlue', className)}>
        <Typography variant="sm" className="select-none">
          {i18n._(t`Swapping with`)}
          <QuestionHelper
            text={i18n._(t`Corellated assets are traded with Stable AMM to optimize price immpact.`)}
          />
        </Typography>
        <Typography variant="sm" className="select-none text-aqua">
          {ammTypeText}
