...
export default function WalletModal({
  ...
  function getModalContent() {
    if (error) {
      return (
        <UpperSection>
          ...
          <HeaderRow style={{ paddingLeft: 20, paddingTop: 20, paddingRight: 20, color: '#ffffff' }}>
          ...
          <div>
            ...
            <ButtonError
              error={true}
              size="sm"
              className="bg-linear-gradient mx-5 mb-8"
              style={{ height: 56, width: '93%' }}
              onClick={() => {
    ...
    return (
      <div className="flex flex-col space-y-4">
        <ModalHeader title={i18n._(t`Connect a Wallet`)} onClose={toggleWalletModal} />
        {/* {walletView === WALLET_VIEWS.ACCOUNT && (
          <HeaderRow>
            <HoverText>{i18n._(t`Connect to a wallet`)}</HoverText>
          </HeaderRow>
        )} */}
        <div className="flex flex-col space-y-6 pr-5 pl-5">
          {walletView === WALLET_VIEWS.PENDING ? (
            ...
            <div className="mb-5">
              <div className="text-white">
                <span className="bg-lightBlueSecondary text-aqua p-3 pt-2 pb-2 mr-4" style={{ fontSize: 14 }}>
                  1
                </span>
                Choose your wallet
              </div>
              <div className="flex space-y-5 overflow-y-auto mt-6">{getOptions()}</div>
              <div
                className="mt-5 bg-deepCove p-4 text-center rounded-sm"
                style={{ fontSize: 12, border: '2px solid #1F357D' }}
              >
                Haven't got a crypto wallet yet?{' '}
                <a href="/" rel="noreferer" target="_blank" className="text-aqua">
  ...
  return (
    <Modal isOpen={walletModalOpen} onDismiss={toggleWalletModal} minHeight={10} maxWidth={600}>
      {getModalContent()}
    </Modal>
  )
}
