...
export const getTokenBalances = (chainId = ChainId.MOONBEAM, address) =>
  fetch(
    `https://api.covalenthq.com/v1/${chainId}/address/${address}/balances_v2/?key=ckey_b1629ebf25d94cc6a4d5abfce31`
  ).then((res) => res.json())

export const getPortfolio = (chainId = ChainId.MOONBEAM, address) =>
  fetch(
    `https://api.covalenthq.com/v1/${chainId}/address/${address}/portfolio_v2/?key=ckey_b1629ebf25d94cc6a4d5abfce31`
  ).then((res) => res.json())

export const getTransfers = (chainId = ChainId.MOONBEAM, address, contractAddress) =>
  fetch(
    `https://api.covalenthq.com/v1/${chainId}/address/${address}/transfers_v2/?key=ckey_b1629ebf25d94cc6a4d5abfce31&contract-address=${contractAddress}&page-size=50000`
  ).then((res) => res.json())
