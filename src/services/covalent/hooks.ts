...
export function useTransfers({ initialData, chainId, address, contractAddress }) {
  const res = useSWR(
    `https://api.covalenthq.com/v1/${chainId}/address/${address}/transfers_v2/?key=ckey_b1629ebf25d94cc6a4d5abfce31&contract-address=${contractAddress}`,
    () => getTransfers(chainId, address, contractAddress),
    { initialData }
  )
  return res
}
