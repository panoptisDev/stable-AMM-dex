...
export const BLOCKS = {
  [ChainId.MOONBEAM]: 'beamswap/moonbeam-blocks',
...
const fetcher = async (chainId = ChainId.MOONBEAM, query, variables = undefined) => {
  return request(`${GRAPH_HOST[chainId]}/subgraphs/name/${BLOCKS[chainId]}`, query, variables)
}

export const getBlock = async (chainId = ChainId.MOONBEAM, variables) => {
  const { blocks } = await fetcher(chainId, blockQuery, variables)

  return { number: Number(blocks?.[0]?.number) }
}

export const getBlocks = async (chainId = ChainId.MOONBEAM, variables) => {
  const { blocks } = await fetcher(chainId, blocksQuery, variables)
  return blocks
}

export const getMassBlocks = async (chainId = ChainId.MOONBEAM, timestamps) => {
...
export const getAverageBlockTime = async (chainId = ChainId.MOONBEAM) => {
