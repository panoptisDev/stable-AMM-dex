...
export function setHardwareWallet(): [boolean, (newSingleHopOnly: boolean) => void] {
  const dispatch = useAppDispatch()

  const hardwareWallet = useAppSelector((state) => state.user.hardwareWallet)

  const setHardwareWalletOnly = useCallback(
    (newSingleHopOnly: boolean) => {
      dispatch(updateHardwareWallet({ hardwareWallet: newSingleHopOnly }))
    },
    [dispatch]
  )

  return [hardwareWallet, setHardwareWalletOnly]
}
