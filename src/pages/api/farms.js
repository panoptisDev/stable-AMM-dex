...
import distributorAbi from '../../constants/abis/beam-distributor.json'
    ...
    for (const pool of poolsInfo) {
      ...
      if (!staticInfo.token1) {
        ret.push({
          ...
          baseSymbol: staticInfo.token0.symbol.toLowerCase().replace('WGLMR', 'movr'),
        ...
        ret.push({
          ...
          baseSymbol:
            token0info.symbol == 'GLINT' || token1info.symbol == 'GLINT'
              ? 'glint'
              ...
              : '',
          baseAmount:
            token0info.symbol == 'GLINT' || token0info.symbol == 'MOVR'
              ? token0amount
              : token1info.symbol == 'GLINT' || token1info.symbol == 'MOVR'
              ? token1amount
