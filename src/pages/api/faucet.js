...
async function faucetSend(req) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    resolve({
      status: 200,
      message: `You will receive GLMR in your wallet soon.`,
    })
  })
}
...
async function checkBridgeUsage(address) {
  const bridges = {
    ETH: `https://bridgeapi.anyswap.exchange/v2/swapin/history/${address}/1284/1/allv2?offset=0&limit=1`,
    BSC: `https://bridgeapi.anyswap.exchange/v2/swapin/history/${address}/1284/56/allv2?offset=0&limit=1`,
  }
...
export default async function handler(req, res) {
  ...
  if (verified) {
    const usedBridge = await checkBridgeUsage(address) // change in prod
    //  const usedBridge = true
    if (usedBridge) {
      ...
      if (movrBalance < 0.07) {
        ...
        if (ref) {
          res.status(200).json({
            status: 400,
            message: 'You are still blacklisted.',
          })
        } else {
        ...
        res.status(200).json({
          status: 403,
          message: 'Your current GLMR balance is above the minimum requirement to use the faucet.',
        })
      }
    } else {
      res.status(200).json({
        status: 403,
        message:
          'This feature is available only for wallets which have used the bridge. If you used the bridge recently, please try again in a couple of minutes',
      })
    }
  } else {
    res.status(200).json({
