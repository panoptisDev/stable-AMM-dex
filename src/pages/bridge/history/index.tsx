...
const Transaction: FC<{ chainId: string; hash: string }> = ({ chainId, hash }) => {
  ...
  const getUrl = () => {
    if (srcChaindId == ChainId.MOONBEAM) {
  ...
  return (
    <div
      className={
        'w-full px-2 py-2 text-left rounded-sm select-none bg-lightBlueSecondary text-primary text-sm md:text-lg'
      }
    >
      <div className="flex flex-col px-2 md:px-0 md:flex-row">
        <div className="flex justify-between flex-row md:flex-none md:w-40">
          <div className="text-white font-bold text-primary items-center md:hidden">
...
export default function Bridge() {
  ...
  return (
    <>
      ...
      <Container maxWidth="2xl" className="space-y-6 history-container mt-6">
          ...
          <div className="p-4 space-y-4 bg-blue" style={{ zIndex: 1, borderRadius: 2 }}>
            <div className="p-4 text-center">
              <div className="flex justify-between space-x-3 items-center">
                <Typography className="text-aqua" component="h3" variant="base">
                  {i18n._(t`View the history of all your bridge interactions`)}
                </Typography>
                <div className="flex flex-row p-1 pr-2 pl-2 bg-darkBlue" style={{ cursor: 'pointer', borderRadius: 3 }}>
                  <RefreshCw
                    onClick={() => setRefresher(refresher + 1)}
                    size={14}
                    className={'font-emphasis hover:font-high-emphasis cursor-pointer text-aqua mt-1 mr-2'}
                  />
                  <Typography className="text-aqua" component="h3" variant="base">
                    {i18n._(t`Refresh`)}
                  </Typography>
            ...
            <BottomGrouping>
              {!account && activeAccount ? (
                ...
                <div className="space-y-2 p-4 rounded-md bg-deepCove mb-2 h-[455px] overflow-y-auto">
                  {allTransactions && Object.keys(allTransactions).length > 0 ? (
                    <>
                      <div className="flex px-2 hidden md:flex text-white font-bold text-jordyBlue items-center">
                    ...
                    <Typography variant="sm" className="text-jordyBlue">
